...
trainprd = ln.predict(train[features])

train_rmse  =  np.sqrt( mean_squared_error(train[target],trainprd))

testprd = ln.predict(clean_test[features])
test_rmse = np.sqrt(  mean_squared_error(clean_test[target],testprd))
